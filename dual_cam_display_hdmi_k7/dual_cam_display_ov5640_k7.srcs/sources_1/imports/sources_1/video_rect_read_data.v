...
(
	...
	input                       rst,
	input[11:0]                 video_left_offset,	// 距离左侧偏移量
	input[11:0]                 video_top_offset,	// 距离顶偏偏移量
...
begin
	if(rst == 1'b1)
		vout_data_r <= {DATA_WIDTH{1'b0}};//fifo慢一拍
...
begin
	if(rst == 1'b1)
		read_req <= 1'b0;
	else if(pos_vs_d0 == 1'b1 & pos_vs == 1'b0) //vertical synchronization edge (the rising or falling edges are OK)//应用上升沿，如果是下降沿就不显示第一个图像
		read_req <= 1'b1;
